...
public class SQLFacturas {
    ...
    public static String obtFactura(String idFactura) {
        return "select le.embfechafac,le.embfecpedido,le.embref,le.embenvio,le.embcliente,lc.clinombre,lp.pobclave,lp.pobnombre,"
                ...
                + " le.embclave,le.embaccion,le.embfoldevdoc,le.embfecdevdoc,le.embfolio"
    ...
    public static String BuscarFacturas(String fecha, String tipoCliente, String estatus, int idEmpresa, String orderBy) {
        return "select le.embclave,le.embestatus,le.embfolio,le.embref,le.embtiporechazo,le.embenvio,le.embcliente,lc.clinombre,lp.pobclave,lp.pobnombre,le.embcajas,le.embvalor "
                ...
                + " to_char(le.embfecembarque,'dd/mm/yyyy')as embfecembarque,to_char(le.embfecentrega,'dd/mm/yyyy')as embfecentrega,le.embcoms,le.embtiporechazo "
                + " from Liembarques le,Liclientes lc,Lipoblaciones lp "
    ...
    public static String BuscarExistencias(String sqlLike, String sqlOrderBy) {
        return "select lp.prdclave,lp.prddescripcion,lp.prdlinea,lp.prdfamilia,lp.prdunidad,li.invreal,li.invresevado,li.invalmacen  "
                ...
                + " and li.invalmacen=?2 and li.invreal>0 " + sqlLike + " " + sqlOrderBy;
    }

    public static String ObtenerFacturas(String sqlBusq) {
        return "select le.embclave,le.embref,le.embfolio,to_char(le.embfechafac,'dd/mm/yyyy')as embfechafac,le.embenvio "
                + " from Liembarques le,Liclientes lc,Lipoblaciones lp "
                + " where le.embempresa=?1 and (lc.clinumero=le.embcliente and lc.cliempresa=le.embempresa) "
                + " and (lp.pobclave=le.embpoblacion) and " + sqlBusq;
    }
